...
scoreboard players operation @e[name=acv_portal_out] acv_color -= @s acv_origin_color
execute store result score @s acv_tmp run data get entity @e[name=acv_portal_out,scores={acv_color=0},limit=1] Rotation[0] .011111111111111111
scoreboard players operation @e[name=acv_portal_out] acv_color += @s acv_origin_color

scoreboard players operation @s acv_delta -= @s acv_tmp

# refer to target portal
scoreboard players operation @e[name=acv_portal_out] acv_color -= @s acv_target_color
execute store result score @s acv_tmp run data get entity @e[name=acv_portal_out,scores={acv_color=0},limit=1] Rotation[0] .011111111111111111
scoreboard players operation @s acv_delta += @s acv_tmp
scoreboard players operation @s acv_delta %= 4 acv_constant
execute at @e[name=acv_portal_out,scores={acv_color=0}] run teleport @s ~ ~ ~
scoreboard players operation @e[name=acv_portal_out] acv_color += @s acv_target_color
